...
export async function POST(request: Request) {
  try {
    ...
    while (iterations < MAX_ITERATIONS) {
      ...
      // Extract all JSON objects from the response using a bracket matching parser
      ...
      for (let i = 0; i < raw.length; i++) {
        if (raw[i] === "{") {
          ...
          if (depth === 0 && start !== -1) {
            try {
              parsedObjects.push(JSON.parse(raw.substring(start, i + 1)));
            } catch (e) {}
          }
        }
      }
      ...
      if (parsedObjects.length === 0) {
        // No JSON found at all — treat as plain text output and return to user
        const aiMessage = await prisma.message.create({
          data: { chatId: chat.id, content: raw, role: "ASSISTANT" },
        });
        return NextResponse.json({ success: true, userMessage, aiMessage, agentSteps });
      }

      let shouldContinue = true;

      // Process each step generated by the LLM
      for (const parsed of parsedObjects) {
        ...
        switch (parsed.step) {
          ...
          case "OBSERVATION":
            ...
            break; // Continue to next parsed object
          ...
          // Stop processing further parsed objects to allow LLM to process the tool result
          shouldContinue = false;
          break;
        }

        case "OUTPUT": {
          // Final answer — save to DB and return immediately
      ...
      // If we executed a tool, we break out of the for-loop to jump to the next while-loop iteration
      if (!shouldContinue) {
        break;
      }
    }

    // If we processed steps but didn't hit a TOOL or OUTPUT, prompt the LLM to continue
    if (shouldContinue) {
      history.push({
        role: "user",
